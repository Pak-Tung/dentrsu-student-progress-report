...
// import { Container, Row, Col, Badge, Alert, Button, FormControl, InputGroup } from "react-bootstrap";
...
//       const response = await updateUserPictureByEmail(userEmail, imageUrl); // Replace with your actual API call
...
//           />
//           {showTextbox && (
//             <div className="position-absolute" style={{ top: '110%', left: '50%', transform: 'translateX(-50%)' }}>
...
const Profile = () => {
  ...
  const { email: userEmail, name: userName, picture: userPicture } = user;
  ...
  const handleImageClick = () => setShowTextbox(true);

  const handleImageSubmit = async () => {
    try {
      const response = await updateUserPictureByEmail(userEmail, {imgUrl: imageUrl});
      if (response.error) {
        setError(response.error);
      } else {
        setUser((prevUser) => ({ ...prevUser, picture: imageUrl }));
